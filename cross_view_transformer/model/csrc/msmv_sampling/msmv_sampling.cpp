...
at::Tensor ms_deform_attn_cuda_c23_forward(
    ...
    const at::Tensor& sampling_loc,  // [B, Q, P, 3]
    const at::Tensor& attn_weight  // [B, Q, P, 2]
) {
    AT_ASSERTM(feat_c2.is_contiguous(), "value tensor has to be contiguous");
...
at::Tensor ms_deform_attn_cuda_c234_forward(
    ...
    const at::Tensor& sampling_loc,  // [B, Q, P, 3]
    const at::Tensor& attn_weight  // [B, Q, P, 3]
) {
    AT_ASSERTM(feat_c2.is_contiguous(), "value tensor has to be contiguous");
    AT_ASSERTM(feat_c3.is_contiguous(), "value tensor has to be contiguous");
    ...
    auto output = at::zeros({ batch_size, num_query, channels, num_point }, feat_c2.options());
    ms_deformable_im2col_cuda_c234(
        feat_c2.data_ptr<float>(),
        feat_c3.data_ptr<float>(),
...
std::vector<at::Tensor> ms_deform_attn_cuda_c234_backward(
    ...
    ms_deformable_col2im_cuda_c234(
        grad_output.data_ptr<float>(),
        feat_c2.data_ptr<float>(),
        feat_c3.data_ptr<float>(),
        feat_c4.data_ptr<float>(),
    ...
    return {
        grad_value_c2, grad_value_c3, grad_value_c4, grad_sampling_loc, grad_attn_weight
    };
}
